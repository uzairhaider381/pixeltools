    ...
    <div className="min-h-[100dvh] flex flex-col font-sans bg-background text-foreground">
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto flex h-14 items-center justify-between px-4 gap-2">
          <Link href="/" className="flex items-center font-bold text-lg tracking-tight text-primary flex-shrink-0">
            <LogoIcon />
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium">
            ...
            <div className="relative group">
              <span className="cursor-pointer text-muted-foreground hover:text-primary transition-colors">Tools ▾</span>
              <div className="absolute top-full left-0 hidden group-hover:flex flex-col bg-popover text-popover-foreground border shadow-lg rounded-md py-2 min-w-[160px] z-50">
                {navLinks.map((link) => (
                  <Link key={link.href} href={link.href} className="px-4 py-2.5 hover:bg-muted text-sm transition-colors">
                    {link.label}
                  </Link>
                ))}
              </div>
